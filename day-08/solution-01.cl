#!/usr/bin/sbcl --script
(require :uiop)
(defparameter *connection-count* (if (second (uiop:command-line-arguments))
                                     (parse-integer (second (uiop:command-line-arguments)))
                                     1000))
        ...
        do (setf next-junction (pop junction-list))
           (dolist (compare-junction junction-list)
             (push (list (calc-distance next-junction compare-junction) next-junction compare-junction)
                   distance-pairs)))
  (setf distance-pairs (last (sort distance-pairs #'> :key #'car) *connection-count*))
  ;; Walk the distance list and join junctions together
  (dolist (pair distance-pairs)
    (merge-circuits circuit-map (cadr pair) (caddr pair))
    (setf circuit-map (remove nil circuit-map)))
  (stable-sort circuit-map (lambda (x y) (< (length x) (length y))))
  (format t "The sizes of the three largest circuits together is ~a."
          (apply #'* (mapcar #'length (last circuit-map 3)))))
